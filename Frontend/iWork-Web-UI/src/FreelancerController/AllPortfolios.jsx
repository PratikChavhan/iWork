...
const AllPortfolios = () => {
  ...
  useEffect(() => {
    fetchPortfolios(); // eslint-disable-next-line
  }, []);

  const fetchPortfolios = async () => {
    try {
      const response = await axios.get(
        `http://localhost:9091/freelancing/api/Portfolio/getPortfolioByUserId/${userId}`
      );
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `http://localhost:9091/freelancing/api/Portfolio/deleteById/${deleteId}`
      );
  ...
  return (
    <div className="table-responsive">
      ...
      {showEditForm && editPortfolio && (
        <EditForm portfolio={editPortfolio} handleClose={handleCloseEditForm} />
      )}
    </div>
  );
};

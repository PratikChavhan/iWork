...
const WorkCard = () => {
  ...
  const handleContactClick = async (id) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:9091/freelancing/api/posts/${id}`
      );
  ...
  const handleReviewSubmit = async () => {
    try {
      await axios.post(
        `http://localhost:9091/freelancing/api/comment/createComment/${selectedPost}/1`,
        { content: newComment }
      );
  ...
  const handleReviewClick = async (id) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:9091/freelancing/api/comment/getAllbyPost/${id}`
      );
      ...
      {loading ? (
        ...
        <div>
          ...
          {selectedPost && (
            <div
              className="modal"
              style={{ display: selectedPost !== null ? "block" : "none" }}
            >
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    ...
                    <div>
                      <button
                        className="btn btn-primary"
                        style={{ marginTop: "0px", alignContent: "center" }}
